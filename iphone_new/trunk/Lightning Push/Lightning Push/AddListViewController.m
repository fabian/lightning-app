...
- (void)doneAddList {
	...
	if (self.checkmark == 1) {
        ...
        [self.lightningAPI addList:self.listNameTextField.text isShared:TRUE];
	} else {
        
        [self.lightningAPI addList:self.listNameTextField.text isShared:FALSE];
        
		[self.delegate finishAddPrivateList:self.listNameTextField.text];
	}
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (cell == nil) {
        ...
        if(indexPath.section == 0) {
            self.listNameTextField = [[UITextField alloc] initWithFrame:CGRectMake(16, 10, cell.frame.size.width-16, cell.frame.size.height-10)];
            self.listNameTextField.placeholder = @"Set name of list";
            
            self.listNameTextField.delegate = self;
            [self.listNameTextField addTarget:self action:@selector(textFieldDidChange:) forControlEvents:UIControlEventEditingDidEnd];
